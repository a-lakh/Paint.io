...
class Experiment(object):
    ...
    """
    def __init__(self, net, train_set, val_set, optimizer_G, optimizer_D_A, optimizer_D_B,lr_scheduler, stats_manager,
...
#                 'ValSet': self.val_set,
    ...
    def GetResults(self,n):
        ...
        if not os.path.exists('./training_test/A'):
            os.makedirs('./training_test/A')
        if not os.path.exists('./training_test/B'):
            os.makedirs('./training_test/B')
            
        with torch.no_grad():
            i=0
            for real_a,real_b in self.val_loader:
                real_a, real_b = real_a.to(self.net.device), real_b.to(self.net.device)
                fake_a,fake_b = self.net(real_a, real_b)
                # Save image files
                tv.utils.save_image(fake_a, 'training_test/A/%d_%04d.png' % (ep,i+1))
                tv.utils.save_image(fake_b, 'training_test/B/%d_%04d.png' % (ep,i+1))
                i+=1
                
                print('Generated images %04d of %04d' % (i, len(self.val_loader)))
                if(i == n):
                    break


    def run(self, num_epochs,lr_scheduler, plot=None):
