...
class Experiment(object):
    ...
    """
    def __init__(self, net, train_set, val_set, stats_manager,
    ...
    def setting(self):
        ...
        return {'Net': self.net,
                ...
                'ValSet': self.val_set,
                'Optimizer_G': self.net.optimizer_G,
                'Optimizer_D_A': self.net.optimizer_D_A,
                'Optimizer_D_B': self.net.optimizer_D_B,
    ...
    def state_dict(self):
        ...
        return {'Net': self.net.state_dict(),
                'Optimizer_G': self.net.optimizer_G.state_dict(),
                'Optimizer_D_A': self.net.optimizer_D_A.state_dict(),
                'Optimizer_D_B': self.net.optimizer_D_B.state_dict(),
                'History': self.history}

    def load_state_dict(self, checkpoint):
        """Loads the experiment from the input checkpoint."""
        self.net.load_state_dict(checkpoint['Net'])
        self.net.optimizer_G.load_state_dict(checkpoint['Optimizer_G'])
        self.net.optimizer_D_A.load_state_dict(checkpoint['Optimizer_D_A'])
        self.net.optimizer_D_B.load_state_dict(checkpoint['Optimizer_D_B'])
        self.history = checkpoint['History']

        # The following loops are used to fix a bug that was
        # discussed here: https://github.com/pytorch/pytorch/issues/2830
        # (it is supposed to be fixed in recent PyTorch version)
        for state in self.net.optimizer_G.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(self.net.device)
        for state in self.net.optimizer_D_A.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(self.net.device)
        for state in self.net.optimizer_D_B.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(self.net.device)
    ...
    def run(self, num_epochs, plot=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for real_a,real_b in self.train_loader:
                ...
                self.net.optimizer_G.zero_grad()
                ...
                self.net.optimizer_G.step()
                ###################################

                ###### Discriminator A ######
                self.net.optimizer_D_A.zero_grad()
                ...
                self.net.optimizer_D_A.step()
                ###################################

                ###### Discriminator B ######
                self.net.optimizer_D_B.zero_grad()
